...
q_alto_A  = [base_A, math.radians(-90), math.radians(90), math.radians(-90), math.radians(-90), 0]
q_baixo_A = [base_A, math.radians(-60), math.radians(60), math.radians(-90), math.radians(-90), 0]

# Ponto B (Lado de Soltar)
q_alto_B  = [base_B, math.radians(-90), math.radians(90), math.radians(-90), math.radians(-90), 0]
q_baixo_B = [base_B, math.radians(-60), math.radians(60), math.radians(-90), math.radians(-90), 0]

# 4. Função Mágica: Move e calcula torque em tempo real
def mover_e_monitorar(q_alvo, F_externa, nome_movimento):
    print(f"\n--- {nome_movimento} ---")
    
    # Inicia o movimento sem travar o código
    rtde_c.moveJ(q_alvo, speed=0.5, acceleration=0.5, asynchronous=True)
    
    while True:
        q_atual = rtde_r.getActualQ()
        
        # Verifica se chegou
        distancia = sum(abs(atual - alvo) for atual, alvo in zip(q_atual, q_alvo))
        if distancia < 0.05:
            print(f"\n[✓] Destino alcançado!")
            break
...
try:
    ...
    mover_e_monitorar(q_alto_A, F_vazia, "1. Indo para Posição de Espera (Vazio)")
    mover_e_monitorar(q_baixo_A, F_vazia, "2. Abaixando para pegar peça (Vazio)")
    ...
    mover_e_monitorar(q_alto_A, F_carga, "3. Levantando a peça (Carga Máxima!)")
    mover_e_monitorar(q_alto_B, F_carga, "4. Movendo para o outro lado (Carga Máxima!)")
    mover_e_monitorar(q_baixo_B, F_carga, "5. Abaixando para soltar (Carga Máxima!)")
    ...
    mover_e_monitorar(q_alto_B, F_vazia, "6. Levantando garra (Vazio)")
    mover_e_monitorar(q_alto_A, F_vazia, "7. Retornando ao Início (Vazio)")
